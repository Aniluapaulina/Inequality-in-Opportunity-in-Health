	...
	* Gesamtvarianz
	...
	local var_pcs_cfa = r(Var)
	   ...
	   * absolute IOp
		...
		predict mcs_hat, xb
		quietly summarize mcs_hat // oder mit [aw=`curweight'] @paulina 
		...
		* absolute IOp
		...
		predict pcs_hat, xb
		quietly summarize pcs_hat // [aw=`curweight'] @paulina 
...
save "$output/para_IOp_yob_timeseries_raw.dta", replace
...
*------------------------------------------------------------------------------
use "$output/para_IOp_yob_timeseries_raw.dta", clear
...
sort year
save "$output/para_IOp_yob_timeseries.dta", replace


/* Vorbereitung für shapley decomposition: year x healthoutcome 
use "$output/para_IOp_yob_timeseries.dta", clear
...
twoway ///
    ...
    (line pt_R2_mcs_cfa year, lpattern(dot) lcolor(navy)) ///
...
*--------------------------------------------------------------------------------------------
* Tabellenexport für Overleaf mit absolute and relative IOp, Sternchen und Fußnote 
* für MCS 
*--------------------------------------------------------------------------------------------
use "$output/para_IOp_yob_timeseries.dta", clear
...
* Datei öffnen
file open fh using "$output/Tables/table_IOP_MCS_yob.tex", write replace
...
* Datei öffnen
file open fh using "$output/Tables/table_IOP_PCS_yob.tex", write replace
