...
* Was mache ich: Ich nehme ppathl (long) als Basis mit unique (pid syear), Migrationshintergrund, Geschlecht und Gewichten und spiele dann 
	...
	* und 1:1 über (pid syear) die Gesundheitsvariablen (health und pequiv)
...
*** Dataset bioparen.dta
	...
	* Infos siblings aus der v39 
	merge 1:1 pid using $v39\bioparen.dta , keepusing(sibl nums numb twin) keep(1 3) 
...
*** Dataset biol.dta 
	...
	* Geschiwister vor 2014
	...
	egen sib_pre2014 = rowtotal(lb0062 lb0063) if lb0062 != . & lb0063 != .
		*br if (lb0062 == . & lb0063 != .) | (lb0062 != . & lb0063 == .)  // 7 Fälle 
		*br if (lb0062 == . | lb0063 == . )
	
	replace siblings_raw = sib_pre2014 if syear < 2014 & (lb0062 != . | lb0063 != . ) // beeinhaltet die 7 Fälle 
...
** Childhood 
	...
	* Mother's & Father's school education
	gen fedu6 = .
		...
		replace fedu6 = . if fsedu == -5
		replace fedu6 = . if missing(fsedu) 
	...
	gen medu6 = .
		...
		replace fedu6 = . if msedu == -5
		replace fedu6 = . if missing(msedu) 
	...
	* Mother's & Father's occupation when individual was 15 years old
		* Variante 1: 
		gen fprof7 = .
			...
			replace fprof7 = . if fprofstat == -1
			replace fprof7 = . if missing(fprof7)
		...
		gen mprof7 = .
			...
			replace mprof7 = . if mprofstat == -1
			replace mprof7 = . if missing(mprof7) 
			
			
		label define prof7_lbl 1 "blue collar" 2 "selfemployed" 3 "white collar" 4 "civil servant" 5 "not working or in training" 6 "no idea" 7 "lebte nicht mehr"
		...
		* Variante 2 ( See Paper by Mel Bartley - that also touches on the National Statistics Socio-Economic Classification. Discussion about which occupation to put in which class/social position; categorisation for example also based on the "degree of manuality" )
		gen fprof12 = . 
			...
			replace fprof12 = . if fprof12 == -1
			replace fprof12 = . if missing(fprof12) 
				// Missing 
		...
		gen mprof12 = . 
			...
			replace mprof12 = . if mprof12 == -1
			replace mprof12 = . if missing(mprof12)
	...
	* Missings umcoden für living* und fsedu msedu
	...
	gen singleparent = .
		...
		replace singleparent = . if (living2 == . & living4 == .) 
	...
	* (Partially) raised in foster-parents household, in a children's home or with relatives other than parents (at least one year until age 15)
	gen otherparent = .
		...
		replace otherparent = . if (living6 == . & living7 == . & living8) 
...
** Place Effects 
	* Federal state born
	...
	label define birthregion_lbl  0 "East" 1 "West" 2 "Abroad" 
...
*/
		...
		gen bweight_0 = w // w ist der Hochrechnungsfaktor (Stichprobengewichte) 
