...
***    Do-file: 5_cohortanalysis2
***	   Description: Mean-based Cohort analysis + Shapley decomposition 	                                     	
...
* Idee: KOhortenanalse für Menschen in Altersspanne von 31-40, die jeweils in die ANalyse nur einmal einbezogen werden. D.h. konkreter., die Kohorten wurden so gewählt, dass zum Beobaachtungszeitraum (3) die Person in der Altersspanne lag. 6 Jahrgänge pro Kohorte und 6 Jahre Abstand in Beobachtungsjahren. 
...
save $output/final_cohort.dta, replace 
...
order cohort ankeryear healthoutcome shap_age relshap_age shap_migback relshap_migback shap_gender relshap_gender shap_msedu relshap_msedu shap_fsedu relshap_fsedu shap_fprofstat relshap_fprofstat shap_mprofstat relshap_mprofstat shap_siblings relshap_siblings shap_singleparent relshap_singleparent shap_otherparent relshap_otherparent shap_birthregion relshap_birthregion shap_urban relshap_urban
...
graph bar (asis) ///
	...
	/// Demographic: Blautöne (migback, gender)
	bar(2,  color("55 126 184")) ///
	bar(3,  color("107 174 214")) ///
	/// SES: Rottöne (msedu, fsedu, fprofstat, mprofstat)
	bar(4,  color("165 0 38"))   /// msedu
	bar(5,  color("215 48 39"))  /// fsedu
	bar(6,  color("244 109 67")) /// fprofstat
	bar(7,  color("253 190 152")) /// mprofstat
	/// Family structure: Grüntöne (singleparent, otherparent, siblings)
	bar(8,  color ("35 139 69"))  /// siblings (dunkleres grün)
	bar(9,  color("116 196 118"))    /// singleparent (mittelgrün)
	bar(10,  color("166 219 160"))   /// otherparent (helleres grün
	...
	bar(12, color("188 189 220")) ///
        ...
        legend(cols(4) size(small) region(lwidth(none)) ///
            order(1 "age" 2 "migback" 3 "gender" 4 "msedu" 5 "fsedu" 6 "fprofstat" ///
		  7 "mprofstat" 8 "singleparent" 10 "otherparent" 11 "birthregion" 12 "urban"))
